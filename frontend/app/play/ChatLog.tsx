...
const ChatLog: React.FC<ChatLogProps> = () => {
  ...
  useEffect(() => {
    ...
    const onNewRoomChat = (data: { name: string; channelId: string }) => {
      setMessages([
        {
          content: `Joined room ${data.name}. ${
            data.channelId
              ? "Chat will be sent to channel: #" + data.channelId
              : ""
          }`,
          username: "",
          color: "green",
    ...
    <div className="absolute top-0 left-0 hidden sm:flex">
      ...
      {expanded && (
        <div className="bg-secondary bg-opacity-80 w-[500px] h-[200px] rounded-br-lg transparent-scrollbar relative p-1">
          <div
            className="cursor-pointer absolute bottom-0 right-0 rounded-tl-lg rounded-br-lg bg-darkblue hover:bg-light-secondary animate-colors bg-opacity-80 p-2"
            onClick={collapse}
          >
